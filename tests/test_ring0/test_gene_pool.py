...
'''
    ...
    def test_unified_score_when_no_overlap(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
    ...
    def test_returns_results_with_empty_context(self, tmp_path):
        ...
        # Empty context → no tag overlap, but unified scoring still returns genes.
    ...
    def embed(self, texts: list[str]) -> list[list[float]]:
        ...
        for text in texts:
            if text in self._mapping:
                ...
                vec = [int(h[i:i+2], 16) / 255.0 for i in range(0, self._default_dim * 2, 2)]
                results.append(vec)
        return results


class TestEmbeddingIntegration:
    def test_embedding_stored_on_add(self, tmp_path):
        db = tmp_path / "test.db"
        provider = _FakeEmbeddingProvider()
        gp = GenePool(db, max_size=10, embedding_provider=provider)
        gp.add(1, 0.85, SAMPLE_SOURCE + "\n# emb_store\n")
        ...
        row = con.execute("SELECT embedding FROM gene_pool LIMIT 1").fetchone()
        con.close()
    ...
    def test_backfill_embeddings(self, tmp_path):
        db = tmp_path / "test.db"
        # First add without embedding provider.
        gp1 = GenePool(db, max_size=10)
        gp1.add(1, 0.85, SAMPLE_SOURCE + "\n# emb_backfill\n")

        # Re-open with embedding provider — should backfill.
        provider = _FakeEmbeddingProvider()
        gp2 = GenePool(db, max_size=10, embedding_provider=provider)
    ...
    def test_get_relevant_strips_embedding(self, tmp_path):
        db = tmp_path / "test.db"
        provider = _FakeEmbeddingProvider()
        gp = GenePool(db, max_size=10, embedding_provider=provider)
        gp.add(1, 0.85, SAMPLE_SOURCE + "\n# emb_strip\n")
    ...
    def test_embedding_boosts_relevance(self, tmp_path):
        """Genes with higher cosine similarity rank higher even with zero tag overlap."""
        db = tmp_path / "test.db"

        # Create two genes with different summaries.
        health_source = '''\
import os, pathlib, time, threading

class ProactiveHealthMonitor:
    """Monitor user health activities and exercise patterns."""

    def track_activity(self, activity_type):
        """Track a physical activity session."""
        pass

    def suggest_exercise(self):
        """Suggest exercise based on user patterns."""
        pass
...
class DnaSequenceAnalyzer:
    """Analyze DNA sequences and genetic mutations."""

    def align_sequences(self, seq_a, seq_b):
        """Align two DNA sequences using dynamic programming."""
        pass

    def find_mutations(self):
        """Find mutations in a DNA sequence."""
        pass
...
if __name__ == "__main__":
    main()
'''
        # Use a provider that returns specific embeddings so health gene
        # is closer to the query than DNA gene.
        query_text = "exercise morning workout"
        health_summary = GenePool.extract_summary(health_source)
        dna_summary = GenePool.extract_summary(dna_source)

        # Query and health are similar (close vectors); DNA is distant.
        provider = _FakeEmbeddingProvider({
            query_text: [1.0, 0.0, 0.0, 0.0],
            health_summary: [0.9, 0.1, 0.0, 0.0],  # cos_sim ≈ 0.994
            dna_summary: [0.0, 0.0, 1.0, 0.0],      # cos_sim = 0.0
        })

        gp = GenePool(db, max_size=10, embedding_provider=provider)
        gp.add(1, 0.90, dna_source)    # higher base score
        gp.add(2, 0.60, health_source)  # lower base score

        query_emb = provider.embed([query_text])[0]
        relevant = gp.get_relevant(query_text, 2, query_embedding=query_emb)

        assert len(relevant) == 2
        # Health gene should rank first due to embedding similarity,
        # despite lower base score and no tag overlap with query.
        assert "health" in relevant[0]["gene_summary"].lower() or "exercise" in relevant[0]["gene_summary"].lower()

    def test_no_embedding_provider_still_works(self, tmp_path):
        """GenePool works fine without embedding_provider (backward compat)."""
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gp.add(1, 0.85, SAMPLE_SOURCE + "\n# no_emb\n")
        relevant = gp.get_relevant("stream analyzer", 1)
...
class TestDeriveGeneName:
    """derive_gene_name() should create a deterministic name from tags."""

    def test_basic(self):
        assert GenePool.derive_gene_name("analyzer anomaly stream") == "analyzer-anomaly-stream"

    def test_sorts_tags(self):
        assert GenePool.derive_gene_name("stream anomaly analyzer") == "analyzer-anomaly-stream"

    def test_takes_first_three(self):
        assert GenePool.derive_gene_name("a b c d e") == "a-b-c"

    def test_empty(self):
        assert GenePool.derive_gene_name("") == "unnamed"

    def test_single_tag(self):
        assert GenePool.derive_gene_name("stream") == "stream"


class TestGetPublishableGenes:
    """get_publishable_genes() should return genes with proven task value."""

    def test_returns_qualifying_genes(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gp.add(1, 0.80, SAMPLE_SOURCE + "\n# pub1\n")
        gene_id = gp.get_top(1)[0]["id"]
        # Give it 2 task hits to qualify.
        gp.record_task_hits([gene_id], generation=1)
        gp.record_task_hits([gene_id], generation=2)
        # Apply task boost to keep score >= 0.6.
        publishable = gp.get_publishable_genes()
        assert len(publishable) == 1
        assert publishable[0]["id"] == gene_id

    def test_excludes_low_score(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gp.add(1, 0.40, DISTINCT_SOURCES[0])
        gene_id = gp.get_top(1)[0]["id"]
        gp.record_task_hits([gene_id], generation=1)
        gp.record_task_hits([gene_id], generation=2)
        publishable = gp.get_publishable_genes()
        assert len(publishable) == 0  # score < 0.6

    def test_excludes_low_task_hits(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gp.add(1, 0.80, SAMPLE_SOURCE + "\n# pub_low\n")
        gene_id = gp.get_top(1)[0]["id"]
        gp.record_task_hits([gene_id], generation=1)  # only 1, need 2
        publishable = gp.get_publishable_genes()
        assert len(publishable) == 0

    def test_excludes_suppressed(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gp.add(1, 0.80, SAMPLE_SOURCE + "\n# pub_supp\n")
        gene_id = gp.get_top(1)[0]["id"]
...
class TestInstallFromHub:
    """install_from_hub() should install genes with deduplication."""

    def test_install_basic(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gene_data = {
            "gene_summary": "class RemoteAnalyzer: ...\n    def analyze(self): ...",
            "tags": ["remote", "analyzer"],
            "score": 0.75,
        }
        result = gp.install_from_hub(gene_data)
        assert result is True
        assert gp.count() == 1
    ...
    def test_dedup_exact(self, tmp_path):
        ...
        gp = GenePool(db, max_size=10)
        gene_data = {
            "gene_summary": "class RemoteAnalyzer: exact dedup test",
            "tags": ["remote", "analyzer"],
            "score": 0.75,
        }
        gp.install_from_hub(gene_data)
        result = gp.install_from_hub(gene_data)
        assert result is False
        assert gp.count() == 1

    def test_dedup_by_tag_jaccard(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        # Install a hub gene first.
        gene1 = {
            "gene_summary": "class StreamProcessor: process data streams",
            "tags": ["stream", "processor", "data", "handler"],
            "score": 0.75,
        }
        gp.install_from_hub(gene1)
        assert gp.count() == 1
        # Try to install a near-duplicate (4 shared tags out of 5 → Jaccard = 4/5 = 0.80 > 0.75).
        gene2 = {
            "gene_summary": "class DataStreamHandler: handle data streams",
            "tags": ["stream", "processor", "data", "handler", "extra"],
            "score": 0.70,
        }
        result = gp.install_from_hub(gene2)
        assert result is False  # too similar (Jaccard = 4/5 = 0.80 > 0.75 threshold)

    def test_dedup_exact_same_tags(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gene1 = {
            "gene_summary": "class AlphaEngine: alpha",
            "tags": ["alpha", "beta", "gamma"],
            "score": 0.75,
        }
        gp.install_from_hub(gene1)
        # Same tags → Jaccard = 1.0 > 0.75.
        gene2 = {
            "gene_summary": "class BetaEngine: beta",
            "tags": ["alpha", "beta", "gamma"],
            "score": 0.70,
        }
        result = gp.install_from_hub(gene2)
    ...
    def test_install_different_gene(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gp.add(1, 0.80, SAMPLE_SOURCE + "\n# existing\n")
        # Install a gene with completely different tags.
        gene_data = {
            "gene_summary": "class WeatherPredictor: predict weather patterns",
            "tags": ["weather", "predictor", "forecast"],
            "score": 0.70,
        }
        result = gp.install_from_hub(gene_data)
        assert result is True
    ...
    def test_respects_pool_max_size(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=1)
        gp.add(1, 0.90, SAMPLE_SOURCE + "\n# full\n")
        gene_data = {
            "gene_summary": "class LowScoreGene: ...",
            "tags": ["low", "quality"],
            "score": 0.50,
        }
        result = gp.install_from_hub(gene_data)
        assert result is False  # score too low to evict

    def test_evicts_lowest_when_higher_score(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=1)
        gp.add(1, 0.40, DISTINCT_SOURCES[0])
        gene_data = {
            "gene_summary": "class HighScoreGene: top quality gene",
            "tags": ["high", "quality"],
            "score": 0.90,
        }
        result = gp.install_from_hub(gene_data)
    ...
    def test_empty_summary_rejected(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gene_data = {"gene_summary": "", "tags": [], "score": 0.5}
        result = gp.install_from_hub(gene_data)
        assert result is False

    def test_respects_blacklist(self, tmp_path):
        db = tmp_path / "test.db"
        gp = GenePool(db, max_size=10)
        gene_data = {
            "gene_summary": "class BlacklistedGene: test",
            "tags": ["blacklisted"],
            "score": 0.75,
        }
        # Install then delete (blacklist).
        gp.install_from_hub(gene_data)
        gene_id = gp.get_top(1)[0]["id"]
        gp.delete_gene(gene_id)
        # Try to re-install.
