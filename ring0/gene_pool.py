...
class GenePool(SQLiteStore):
    ...
    def get_publishable_genes(self) -> list[dict]:
        """Return genes with proven task value suitable for hub sharing.

        Criteria: total_task_hits >= 2, score >= 0.6, not suppressed.
        """
        suppressed = self.get_suppressed_gene_ids()
        with self._connect() as con:
            rows = con.execute(
                "SELECT id, generation, score, gene_summary, tags, "
                "total_task_hits, embedding "
                "FROM gene_pool "
                "WHERE total_task_hits >= 2 AND score >= 0.6"
            ).fetchall()
        result = []
        for row in rows:
            gene = dict(row)
            if gene["id"] in suppressed:
                continue
            result.append(gene)
        return result

    @staticmethod
    def derive_gene_name(tags_str: str) -> str:
        """Derive a shareable gene name from tag string.

        Sort tags, take first 3, join with ``-``.
        E.g. ``"analyzer anomaly stream"`` → ``"analyzer-anomaly-stream"``.
        """
        if not tags_str:
            return "unnamed"
        parts = sorted(tags_str.split())
        return "-".join(parts[:3]) or "unnamed"

    def install_from_hub(self, gene_data: dict) -> bool:
        """Install a gene downloaded from the hub.

        Deduplicates by tag Jaccard similarity (>0.75 = skip).
        Returns True if installed.
        """
        gene_summary = gene_data.get("gene_summary", "")
        if not gene_summary:
            return False

        tags_raw = gene_data.get("tags", [])
        if isinstance(tags_raw, list):
            tags_str = " ".join(sorted(tags_raw))
        else:
            tags_str = str(tags_raw)
        new_tags = set(tags_str.split()) if tags_str else set()

        # Compute source hash from gene_summary (hub genes have no raw source).
        source_hash = hashlib.sha256(gene_summary.encode()).hexdigest()

        # Embedding from hub (may be empty).
        embedding_json = gene_data.get("embedding", "")

        score = gene_data.get("score", 0.5)
        ...
        with self._connect() as con:
            ...
            # Check near-duplicate by tag Jaccard.
            if new_tags:
                rows = con.execute(
                    "SELECT id, tags FROM gene_pool WHERE tags IS NOT NULL AND tags != ''"
                ).fetchall()
                for row in rows:
                    existing_tags = set(row["tags"].split())
                    intersection = len(new_tags & existing_tags)
            ...
            count = con.execute(
                "SELECT COUNT(*) AS cnt FROM gene_pool"
            ).fetchone()["cnt"]

            if count >= self.max_size:
                # Evict lowest if new score is higher.
                min_row = con.execute(
                    "SELECT id, score FROM gene_pool ORDER BY score ASC LIMIT 1"
                ).fetchone()
                if not min_row or score <= min_row["score"]:
                    return False
                con.execute("DELETE FROM gene_pool WHERE id = ?", (min_row["id"],))

            con.execute(
                "INSERT INTO gene_pool "
                "(generation, score, source_hash, gene_summary, tags, embedding) "
                "VALUES (?, ?, ?, ?, ?, ?)",
                (0, score, source_hash, gene_summary, tags_str, embedding_json),
            )
            log.info("Gene pool: installed gene from hub (score=%.2f, tags=%s)",
                     score, tags_str[:40])
            return True
